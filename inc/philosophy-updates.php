...
function philosophy_fetch_update() {
	...
	$response = wp_remote_get(
		...
		array(
			'timeout'    => 8,
			'user-agent' => philosophy_update_user_agent(),
		)
	);
 ...
 * @return string
 */
function philosophy_update_user_agent() {
	return 'Philosophy/' . PHILOSOPHY_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function philosophy_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = philosophy_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'philosophy_update_request_args', 10, 2 );
