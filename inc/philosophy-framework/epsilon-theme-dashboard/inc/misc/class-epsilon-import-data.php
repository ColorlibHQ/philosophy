...
class Epsilon_Import_Data {
	...
	public static function import_selective_data( $args = array() ) {
		...
		$instance->path = get_template_directory() . '/inc/philosophy-framework/epsilon-theme-dashboard/assets/data/demo.json';
	...
	public function import_sections( $id = '', $type = '' ) {
		...
		$cp = $this->check_contact_page();

		if( 'ok' !== $cp ){
			return 'nok';
		}

		$ap = $this->check_about_page();
	 ...
	 */
	public function check_contact_page() {
	...
	/**
	 * Check if we have a about page
	 */
	public function check_about_page() {
		...
		if( $id ){
			update_post_meta( $id, '_wp_page_template', 'page-about.php' );
	...
	public function upload_logo( $image ) {
		...
		$file = array(
			'name'     => basename( 'colorlib-logo' . rand( 1, 123123123 ) ) . '.png',
			'tmp_name' => $tmp,
		);
		if ( is_wp_error( $tmp ) ) {
