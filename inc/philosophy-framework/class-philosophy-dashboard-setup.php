...
class Philosophy_Dashboard_Setup {
	/**
	 * Theme array
	 *
	 * @var array
	...
	/**
	 * Philosophy_Dashboard_Setup constructor.
	 *
	 * @param array $theme
	 */
	 ...
	 * @param array $theme
	 *
	 * @return Philosophy_Dashboard_Setup
	 */
	public static function get_instance( $theme = array() ) {
		static $inst;
		if ( ! $inst ) {
			$inst = new Philosophy_Dashboard_Setup( $theme );
		}
	...
	public function add_admin_notice() {
		...
		$imported_demo = Philosophy_Notify_System::check_installed_data();
		if ( $imported_demo ) {
			return;
		}

		if ( empty( $this->notice ) ) {
			$this->notice .= '<img src="' . esc_url( get_template_directory_uri() ) . '/inc/philosophy-framework/epsilon-theme-dashboard/assets/images/colorlib-themes-logo-black.png" class="epsilon-author-logo" />';
			...
			$this->notice .=
				sprintf( /* Translators: Notice */
					esc_html__( 'Welcome! Thank you for choosing %3$s! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'philosophy' ),
	...
	public function get_steps() {
		return array(
			...
			array(
				...
				'demos'    => PHILOSOPHY_DIR_PATH_FRAM . 'customizer/demo/demo.json',
				'buttons'  => array(
					'next' => array(
						'action' => 'next',
						'label'  => __( 'Next <span class="dashicons dashicons-arrow-right-alt2"></span>', 'philosophy' ),
			...
			array(
				...
				'content'  => array(
					'paragraphs' => array(
						...
						__( 'Allow MachoThemes to track theme usage.', 'philosophy' ),
	...
	public function get_actions() {
		...
		return array(
			array(
				...
				'check'       => Philosophy_Notify_System::check_installed_data(),
				'state'       => false,
				'actions'     => array(
					array(
			...
			array(
				...
				'title'       => Philosophy_Notify_System::plugin_verifier( 'contact-form-7', 'title', 'Contact Form 7', 'verify_cf7' ),
				'description' => Philosophy_Notify_System::plugin_verifier( 'contact-form-7', 'description', 'Contact Form 7', 'verify_cf7' ),
				'plugin_slug' => 'contact-form-7',
				'state'       => false,
				'check'       => defined( 'WPCF7_VERSION' ),
				'actions'     => array(
					array(
						'label'   => Philosophy_Notify_System::plugin_verifier( 'contact-form-7', 'installed', 'Contact Form 7', 'verify_cf7' ) ? __( 'Activate Plugin', 'philosophy' ) : __( 'Install Plugin', 'philosophy' ),
						'type'    => 'handle-plugin',
						'handler' => Philosophy_Notify_System::plugin_verifier( 'contact-form-7', 'installed', 'Contact Form 7', 'verify_cf7' ),
					),
				),
			),
		);
		...
		return array(
			array(
				...
				'content' => array(
					...
					array(
						'title'     => esc_html__( 'Lend a hand and share your thoughts', 'philosophy' ),
						'paragraph' => vsprintf(
						// Translators: 1 is Theme Name, 2 is opening Anchor, 3 is closing.
							__( 'We worked hard on making %1$s the best one out there. We are interested in hearing your thoughts about %1$s and what we could do to make it even better.<br/> <br/>', 'philosophy' ),
							array(
								$theme->get( 'Name' ),
							)
						),
						'action'    => '<a class="button button-feedback" target="_blank" href="https://bit.ly/feedback-philosophy">Have your say</a><br/>',
						'type'      => 'standout',
					),
				),
			),
