...
class Epsilon_Welcome_Screen {
		...
		if ( is_admin() ) {
			...
			wp_localize_script(
				...
				array(
					...
					'no_required_actions_text' => esc_html__( 'Hooray! There are no required actions for you right now.', 'philosophy' ),
					'ajax_nonce'               => wp_create_nonce( 'welcome_nonce' ),
					'activating_string'        => esc_html__( 'Activating', 'philosophy' ),
					'body_class'               => 'appearance_page_' . $this->theme_slug . '-welcome',
					'no_actions'               => esc_html__( 'Hooray! There are no required actions for you right now.', 'philosophy' ),
				)
			);
		}
	}
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'philosophy' ), $this->theme_name ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome&tab=recommended-actions' ) ) . '" class="button button-primary button-hero" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'philosophy' ), $this->theme_name ) . '</a></p>';
		...
		$notifications->add_notice(
			array(
				'id'      => 'philosophy_welcome_notification',
	...
	public function welcome_screen_menu() {
		...
		$title = sprintf( esc_html__( 'About %1$s', 'philosophy' ), esc_html( $this->theme_name ) );
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			'getting-started'     => array(
				...
				'label' => __( 'Getting Started', 'philosophy' ),
			...
			'recommended-actions' => array(
				...
				'label' => __( 'Recommended Actions', 'philosophy' ),
	...
	private function check_plugin( $slug = '' ) {
		$arr = array(
			...
			'label'     => __( 'Install and Activate', 'philosophy' ),
		);
		...
		if ( $arr['installed'] ) {
			...
			$arr['label'] = __( 'Activate now', 'philosophy' );
		}

		if ( $arr['active'] ) {
			$arr['needs'] = 'deactivate';
			$arr['class'] = 'deactivate-now button button-primary';
			$arr['label'] = __( 'Deactivate now', 'philosophy' );
		}
